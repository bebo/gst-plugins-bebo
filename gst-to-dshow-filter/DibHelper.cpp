...
uint64_t GetCounterSinceStartMillisRounded(__int64 sinceThisTime) // see above for some timing
{
  LARGE_INTEGER li;
  QueryPerformanceCounter(&li);
  ASSERT_RAISE(PCFreqMillis != 0.0); // make sure it's been initialized...this never happens
  return std::llround(long double(li.QuadPart - sinceThisTime) / PCFreqMillis); //division kind of forces us to return a double of some sort...
} // LODO do I really need long double here? no really.

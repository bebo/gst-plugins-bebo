...
HRESULT CPushPinDesktop::FillBuffer(IMediaSample* media_sample, DxgiFrame** out_dxgi_frame)
{
  ...
  while (!got_frame) {
    ...
    if (got_frame) {
      CopyTextureToStaging(dxgi_frame);
  ...
  media_sample->SetTime(&start_frame, &end_frame);
...
HRESULT CPushPinDesktop::CopyTextureToStaging(DxgiFrame* frame) {
  ...
  frame->texture_timestamp = last_frame_;
...
DxgiFrame::DxgiFrame() 
  : dxgi_handle(NULL), nr(0), index(0), 
    texture_mapped_to_memory(false), texture_timestamp(0L) {
}

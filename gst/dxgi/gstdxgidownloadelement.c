...
#if GST_DXGI_HAVE_PLATFORM_EGL && GST_DXGI_HAVE_DMABUF
...
gst_gl_download_element_set_caps (GstBaseTransform * bt, GstCaps * in_caps,
    GstCaps * out_caps)
{
  GstGLDownloadElement *dl = GST_DXGI_DOWNLOAD_ELEMENT (bt);
  ...
  if (gst_caps_features_contains (features, GST_CAPS_FEATURE_MEMORY_DXGI_MEMORY)) {
    ...
    GstPadDirection direction, GstCaps * caps, GstCaps * filter)
{
  ...
  if (direction == GST_PAD_SRC) {
    tmp = _set_caps_features (caps, GST_CAPS_FEATURE_MEMORY_DXGI_MEMORY);
...
_try_export_dmabuf (GstGLDownloadElement * download, GstBuffer * inbuf)
{
  ...
  glmem = GST_DXGI_MEMORY_CAST (gst_buffer_peek_memory (inbuf, 0));
  if (glmem) {
    GstGLContext *context = GST_DXGI_BASE_MEMORY_CAST (glmem)->context;
    if (gst_gl_context_get_gl_platform (context) != GST_DXGI_PLATFORM_EGL)
      return NULL;
  }
  ...
  for (i = 0; i < gst_buffer_n_memory (inbuf); i++) {
    ...
    glmem = GST_DXGI_MEMORY_CAST (gst_buffer_peek_memory (inbuf, i));
...
#endif /* GST_DXGI_HAVE_PLATFORM_EGL && GST_DXGI_HAVE_DMABUF */
...
gst_gl_download_element_decide_allocation (GstBaseTransform * trans,
    GstQuery * query)
{
  GstGLDownloadElement *download = GST_DXGI_DOWNLOAD_ELEMENT_CAST (trans);
...
{
  GstGLDownloadElement *download = GST_DXGI_DOWNLOAD_ELEMENT_CAST (object);
